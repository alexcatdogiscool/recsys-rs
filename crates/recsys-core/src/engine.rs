...
where
    ...
    QB: QueryBuilder<R::Representation>,
{
    pub fn recommend(&mut self, profile: &Profile, limit_per_source: u32) -> anyhow::Result<Vec<(Item, f32)>> {
        let query = self.query_builder.build_query(&profile.engage_history, self.scraper.query_style_hint())?;
        
        let mut candidates = self.scraper.scrape(&query, limit_per_source)?;

        // remove already seen entries
        let seen: std::collections::HashSet<&str> = profile.engage_history.iter()
            .chain(&profile.ignore_history)
            .map(|i| i.id.as_str())
            .collect();
        let candidates: Vec<Item> = candidates.into_iter()
